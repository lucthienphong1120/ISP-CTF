import base64
def solve_ispclub(indat):
    return indat[::8]
data = "1ch4ll="
def solve_crypt(indat):
    out=""
    indat = indat[3:]
    indat = indat[::-1]
    cnt=1
    while (len(indat)):
        out+=indat[1]
        indat=indat[len(str(cnt))+1:]
        cnt+=1
    return out
def solve_obfuscate(indata):
    indata = indata.replace("imustDOTHISCHALL011014",'')
    indata = indata.replace("ispclub6910832",'')
    indata = indata.encode('utf-8')
    return str(base64.b64decode(indata))
print(solve_obfuscate(solve_crypt(solve_ispclub(data))))